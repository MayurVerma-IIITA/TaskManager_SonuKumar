...
const RegisterForm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setErrors({
        ...fieldErrors,
        form: Object.keys(fieldErrors).length ? "" : getApiErrorMessage(error, "Registration failed")
      });
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      >
        {submitting ? <Spinner size="sm" className="border-white/40 border-t-white" /> : "Create account"}
      </button>
